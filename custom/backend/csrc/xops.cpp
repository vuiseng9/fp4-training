...
TORCH_LIBRARY(xops, m) {
  ...
  m.def("cublaslt_mm_nvfp4(int gemm_id, ScalarType DoutType, Tensor A, Tensor scaleA, Tensor B, Tensor scaleB, Tensor? b=None) -> (Tensor Dt, Tensor scaleDout)"); // A, B row-major, only TN layout is supported
}