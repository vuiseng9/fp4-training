...
TORCH_LIBRARY(xops, m) {
  ...
  m.def("cublaslt_mm_nvfp4(int gemm_id, ScalarType DoutType, Tensor A, Tensor scaleA, Tensor B, Tensor scaleB, Tensor? b=None) -> (Tensor Dt, Tensor scaleDout)"); // A, B row-major for all 3 modes. 0=fwd_gemm, 1=input grad gemm, 2=weight grad gemm
  m.def("cublaslt_mm_nvfp4_packedAB(int gemm_id, ScalarType DoutType, Tensor A, Tensor scaleA, Tensor B, Tensor scaleB, Tensor? b=None) -> (Tensor Dt, Tensor scaleDout)"); // A, B row-major for all 3 modes. 0=fwd_gemm, 1=input grad gemm, 2=weight grad gemm
