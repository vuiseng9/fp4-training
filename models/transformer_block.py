...
class FusedNormTransformerBlock(nn.Module):
    def __init__(self, E, F, H, dropout=0.1, impl="LayerNorm"):
        super().__init__()
        self.E = E
        self.F = F
        self.H = H
        ...
        self.attn = FusedNormAttentionBlock(E, H, dropout, impl=impl)

        self.ffn = nn.ModuleDict({
            "norm_up_proj": te.LayerNormLinear(E, F, normalization=impl),
            "act": nn.GELU(),
            "act_dropout": nn.Dropout(dropout),
            "down_proj": te.Linear(F, E),
            "dropout": nn.Dropout(dropout),
        })
...
class FusedNormAttentionBlock(nn.Module):
    def __init__(self, E, H, dropout=0.1, impl="LayerNorm"):
        super().__init__()
        assert E % H == 0, "head size is not multiple of embedding size"
        ...
        self.norm_qkv_proj = te.LayerNormLinear(E, 3 * E, normalization=impl)
        self.o_proj = te.Linear(E, E)
        self.dropout = nn.Dropout(dropout)
    ...
    def forward(self, x):
        ...
        x   = self.norm_qkv_proj(x).split(E, dim=-1)

        q   = x[0].view(B, L, self.H, self.dh).transpose(1, 2)                   # B, H, L, dh
        k_t = x[1].view(B, L, self.H, self.dh).transpose(1, 2).transpose(-2, -1) # B, H, dh, L
        v   = x[2].view(B, L, self.H, self.dh).transpose(1, 2)                   # B, H, L, dh
