...
def parse_args():
    parser = argparse.ArgumentParser(description="Train TinyViT on MNIST with various Linear implementations")
...
def main():
    ...
    model = TinyViT(linear_impl=args.impl).to(DEVICE)

    param_2d, param_non_2d = [], []
    for _, p in model.named_parameters():
        if not p.requires_grad:
            continue

        # Muon is meant for 2D weight matrices (Linear / projection weights)
        if p.ndim == 2:
            param_2d.append(p)
        else:
            param_non_2d.append(p)


    muon_opt = torch.optim.Muon(param_2d, lr=LR)
    adam_opt = torch.optim.AdamW(param_non_2d, lr=LR)
